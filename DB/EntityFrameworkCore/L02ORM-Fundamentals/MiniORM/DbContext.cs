	...
	public abstract class DbContext
    {
        ...
        public void SaveChanges()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                ...
                {
                    var message = $"{invalidEntities.Length} invalid entities found in {dbSet.GetType().Name}!";
            ...
            using(new ConnectionManager(connection))
            {
                using(var transaction = this.connection.StartTransaction())
                {
                    foreach (var dbSet in dbSets)
                    {
                        ...
                        var persistedMethod = typeof(DbContext).GetMethod("Persist", BindingFlags.Instance | BindingFlags.NonPublic)
        ...
        private void InitializeDbSets()
        {
            foreach (var dbSet in this.dbSetProperties)
            {
                ...
                var populatedDbSetGeneric = typeof(DbContext).GetMethod("PopulateDbSet", BindingFlags.Instance | BindingFlags.NonPublic)
                                                             .MakeGenericMethod(dbSetType);
            ...
            where TEntity : class, new()
        {
            ...
            foreach (var collection in collections)
            {
                ...
                var mapCollectionMethod = typeof(DbContext).GetMethod("MapCollectons", BindingFlags.Instance | BindingFlags.NonPublic)
        ...
        private void MapCollection<TDbSet, TCollection>(DbSet<TDbSet> dbSet, PropertyInfo collectionPropery)
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            foreach (var entity in dbSet)
            {
                ...
                var navigationEntities = navigationDbSet.Where(ne => primaryKey.GetValue(ne).Equals(primaryKeyValue))
                                                        .ToArray();

                ReflectionHelper.ReplaceBackingField(entity, collectionPropery.Name, navigationEntities);
            }
        }
